...
Node DynamicArray::getElement(int pos) const
{
	//teta(1)
	if (pos <= n && n > 0)
		return this->v[pos];
...
Node DynamicArray::setElement(int pos, Node newElem)
{
	//teta(1)
	if (pos <= n && n > 0)
	{
		Node oldElem = this->v[pos];
		this->v[pos] = newElem;
		/*
...
void DynamicArray::addToEnd(Node newElem)
{
	//O(n)
	if (n == capacity)
	{
...
void DynamicArray::addToPosition(int pos, Node newElem)
{
	//O(n)
	if (pos <= n)
	{
...
Node DynamicArray::remove(int pos)
{
	if (pos <= n)
	{
		if (pos == n - 1) {
...
int DynamicArray::findElem(Node e)
{
	for (int i = 0; i < this->n; ++i)
	{
		if (v[i].info == e.info && v[i].next == e.next && v[i].prev == e.prev)
			return i;
	}
